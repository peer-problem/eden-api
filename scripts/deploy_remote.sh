...
cat >/etc/systemd/system/eden-phase1-query-plans.service <<'UNIT'
[Unit]
Description=EDEN Phase 1 bounded query-plan gate
...
EnvironmentFile=/opt/eden/shared/.env
UnsetEnvironment=MIGRATION_DB_USER MIGRATION_DB_PASSWORD VPS_PASSWORD
ExecStart=/opt/eden/current/.venv/bin/python scripts/phase1_query_plans.py
StandardOutput=append:/opt/eden/phase1-evidence/query-plans.jsonl
StandardError=journal
NoNewPrivileges=true
PrivateTmp=true
ProtectHome=true
...
cat >/etc/systemd/system/eden-phase1-query-plans.timer <<'UNIT'
[Unit]
Description=Verify EDEN Phase 1 query plans daily

[Timer]
OnCalendar=*-*-* 02:45:00
Persistent=true
Unit=eden-phase1-query-plans.service

[Install]
WantedBy=timers.target
UNIT

cat >/etc/systemd/system/eden-db-maintenance-report.service <<'UNIT'
[Unit]
Description=EDEN weekly ANALYZE candidates and slow-query report
After=mariadb.service
Requires=mariadb.service
...
EnvironmentFile=/opt/eden/shared/.env
UnsetEnvironment=MIGRATION_DB_USER MIGRATION_DB_PASSWORD VPS_PASSWORD
ExecStart=/opt/eden/current/.venv/bin/python scripts/phase1_db_maintenance.py
StandardOutput=append:/opt/eden/phase1-evidence/database-maintenance.jsonl
StandardError=journal
NoNewPrivileges=true
PrivateTmp=true
ProtectHome=true
...
cat >/etc/systemd/system/eden-db-maintenance-report.timer <<'UNIT'
[Unit]
Description=Collect EDEN database maintenance evidence weekly

[Timer]
OnCalendar=Sun *-*-* 03:30:00
Persistent=true
Unit=eden-db-maintenance-report.service
...
systemctl start eden-phase1-query-plans.service
systemctl start eden-db-maintenance-report.service
...
systemctl enable --now eden-phase1-soak.timer \
  ...
  eden-phase1-query-plans.timer eden-db-maintenance-report.timer \
