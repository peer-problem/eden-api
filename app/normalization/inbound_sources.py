...
def normalize_airport_country_run(
        ...
        for raw in _run_records(session, run_id, AIRPORT_COUNTRY_SOURCE):
            try:
                ...
                for row in public_data_items(_document(raw)):
                    flights = _decimal(row, "arrFlight")
                    if flights is not None and flights < 0:
                        raise ValueError("arriving flight count cannot be negative")
                    ...
                    values = {
                        ...
                        "period_start": period,
                        "grain": "month",
                        "arriving_flights": int(flights) if flights is not None else None,
                        "passengers": None,
                        "schedule": None,
...
def count_next_week_flights(
    ...
    for row in rows:
        ...
        for offset in range(7):
            ...
            operates = (
                _text(row, WEEKDAY_FIELDS[current.weekday()]) or "N"
            ).upper() == "Y"
            if first <= current <= last and operates:
                occurrences += 1
                day = grouped[country]["daily"][current.isoformat()]
                day["flights"] += 1
                day["routes"][airport_code] += 1
        grouped[country]["flights"] += occurrences
        grouped[country]["routes"][airport_code] += occurrences
    return {
        country: {
...
def normalize_airport_weekly_run(
    session_factory: sessionmaker[Session], run_id: str
) -> int:
    normalized = 0
    calculated_at = datetime.now(UTC).replace(tzinfo=None)
    ...
    with session_factory.begin() as session:
        ...
        for raw in records:
            try:
                rows.extend(public_data_items(_document(raw)))
        ...
        for country, values_by_country in grouped.items():
            ...
            values = {
                ...
                "observed_at": max(_database_time(raw.observed_at) for raw in records),
                "source_updated_at": period,
                "ingested_at": max(_database_time(raw.ingested_at) for raw in records),
                "calculated_at": calculated_at,
    ...
    with session_factory.begin() as session:
        for raw in _run_records(session, run_id, FX_SOURCE):
            try:
                ...
                rate_date = _date(
                    _text(body, "rate_date", required=True) or "", ("%Y-%m-%d",)
                )
                for row in body["rates"]:
                    ...
                    _provenance(
                        session, "fx_observation", observation_id, (raw.raw_record_id,)
                    )
                    normalized += 1
            except Exception as exc:
    ...
    with session_factory.begin() as session:
        ...
        for raw in _run_records(session, run_id, BOK_SOURCE):
            ...
            try:
                ...
                expected_code = {"receipt": "2C1Y00", "expenditure": "2C2Y00"}.get(
                    metric
                )
            ...
            for row in rows:
                try:
                    ...
                    value_usd = (value * Decimal("1000000")).quantize(Decimal("0.01"))
            ...
            values = {
                ...
                "quality_flags": (
                    ...
                    if complete
                        ...
                        "missing_expenditure" if expenditure is None else "missing_receipt"
                    ]
                ),
            }
            session.execute(
                insert(TourismBalanceObservation)
                .values(**values)
                .on_duplicate_key_update(**values)
            )
            observation_id = session.scalar(
...
def normalize_tourism_admission_run(
    session_factory: sessionmaker[Session], run_id: str
) -> int:
    normalized = 0
    calculated_at = datetime.now(UTC).replace(tzinfo=None)
        ...
        for raw in _run_records(session, run_id, TOURISM_ADMISSION_SOURCE):
            try:
                for row in public_data_items(_document(raw)):
                    ...
                    domestic = _decimal(row, "csNatCnt")
                    foreign = _decimal(row, "csForCnt")
                    counts = {
                        "domestic": int(domestic) if domestic is not None else None,
                        "foreign": int(foreign) if foreign is not None else None,
                        "all": (
                            int(domestic + foreign)
