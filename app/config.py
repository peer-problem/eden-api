...
class Settings(BaseSettings):
    ...
    def reject_local_development_database(self) -> Settings:
        ...
        if self.DB_SSH_TUNNEL:
            if not is_local:
                raise ValueError("Development SSH forwarding requires a loopback endpoint")
            return self
        if is_local:
            raise ValueError(
    ...
    def validate_production_database_security(self) -> Settings:
        ...
        if not self.DB_PASSWORD.get_secret_value():
            raise ValueError("Production verified TLS requires a nonempty database password")
    ...
    def database_url_for(self, user: str, password: SecretStr) -> str:
        ...
        return (
            f"mysql+mariadbconnector://{quote_plus(user)}:{quote_plus(password.get_secret_value())}"
