...
- 추천 전용 스냅샷 생성과 스케줄러 등록을 제거했다. 장소 상세 조회와 원천 수집은 유지한다.
- 고정 국가 통계 seed와 임의 점수 및 과거 실적 기반 전망 대체를 제거했다. 수집 근거가 없으면 null 또는 unavailable로 응답한다. DB 정리 마이그레이션 `20260917_0011`은 별도 적용이 필요하다.

## 0.3.0 (2026-09-17)

운영 배포 release `20260917T014213Z`. 이 판의 목표는 공개 API 8개가 설계한 데이터를 실제로 수집·조합해 내도록 만드는 것이다. 엔드포인트별 현황은 저장소 루트 README의 "API 제공 범위"에 있다.
