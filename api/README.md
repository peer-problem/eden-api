...
| `GET` | `/v1/places` | Place list for an area |
...
| `GET /v1/forecasts/visitors` | Sigungu codes get the official KTO concentration forecast averaged over the area's attractions (`method: official`, `sample_count`); province (sido) codes get the mean over every attraction in their sigungu (`basis` names the sigungu and attraction counts); areas without any official forecast return `unavailable`; weather, festivals and holidays per day. `requested_area_code` echoes the identifier you sent, `data_area_code` and `spatial_resolution` describe the area the numbers belong to | Forecast horizon 30 days; weather refreshed every 3 hours for one grid per province; festivals weekly; holidays monthly | `expected_visitors`, `confidence`, `adjustment_factors` always `null`; `nx`/`ny` other than the province grid are `unavailable` |
...
| `GET /v1/places` | Canonical places of a province or sigungu (a province includes its sigungu), ordered by title; `lang` picks the title language with Korean fallback per item; `q` matches a title substring; `limit`/`offset` page through `total` | Same TourAPI sources as place details | Places without a Korean title are not listed; `total` counts every match, not only the page |
