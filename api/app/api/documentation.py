...
- API는 DB에 게시된 데이터만 조회합니다. 요청 중 외부 수집이나 LLM 호출을 실행하지 않습니다.

```bash
curl --get 'https://api.edenapi.org/v1/trends' \
  --data-urlencode 'keyword=Korea travel' --data 'country=US&social_sources=youtube'
curl 'https://api.edenapi.org/v1/markets/inbound?countries=JP&countries=CN&include=fx&currency=JPY'
```
...
요청 지역의 관측이 없으면 unavailable이며 상위 지역의 값으로 대체하지 않습니다.

방문 전망 기본값은 7일입니다. 1일부터 30일까지 명시적 요청을 받습니다.
공식 예측이 없으면 unavailable이며 과거 방문 실적으로 전망을 만들지 않습니다.
`expected_visitors`와 검증되지 않은 `confidence`는 null입니다.
...
고정 국가 seed와 임의 가중치의 `inbound_score`는 제공하지 않습니다.
환율은 `currency`를 지정한 경우에만 수집된 통화별 관측을 반환합니다.
...
PARAMETER_DESCRIPTIONS = {
    ...
    "days": ("전망 일수. 기본 7일, 1~30일. 공식 전망이 없는 날짜는 unavailable입니다."),
    ...
    "currency": (
        "ISO 4217 통화 코드. 환율 조회 시 지정해야 합니다. 미보유 통화는 unavailable입니다."
    ),
...
FIELD_DESCRIPTIONS = {
    ...
    "method": "official은 공식 전망입니다. 근거가 없으면 null입니다.",
    "sample_count": "지역 평균 계산에 사용한 공식 관광지 전망 수.",
    "basis": "공식 전망의 집계 근거.",
    ...
    "passengers": (
        "인천공항 국가별 도착 여객 수(명, 유임 및 무임 및 환승 합계). 여객 통계를 수집하기 전 "
        "달은 null입니다."
    ),
...
FIELD_DESCRIPTIONS.update(
    {
        ...
        "festivals": (
            "실제 원천의 해당 구간 행사 참고 정보. 원천이 최근에 수집된 날짜에 행사가 없으면 "
            "빈 목록, 수집 범위 밖이면 null. 전망 인원 보정에 사용하지 않음."
        ),
        ...
        "fx": "명시적으로 선택한 통화의 수집 환율. currency 미지정 시 null.",
        ...
        "holiday": (
            "공식 공휴일 참고 정보. 원천이 최근에 수집된 날짜에 공휴일이 아니면 false, "
            "수집 범위 밖이면 null."
        ),
        ...
        "lang": "조회할 관광지의 언어. 기본 ko.",
        "limit": "최대 반환 건수. API별 허용 범위를 확인하세요.",
        ...
        "rank": "원천 거점 또는 연관 장소의 순위. 1부터 시작.",
        ...
        "score": "연관 관광지는 점수 없이 원천 rank만 제공. 상승 키워드는 관측 변화율.",
