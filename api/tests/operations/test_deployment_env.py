...
from app.operations.environment import generate, render_environment
...
def test_generated_env_preserves_multiline_and_literal_shell_characters(tmp_path: Path) -> None:
    ...
    deploy = Path(__file__).resolve().parents[3] / ".ops/deploy.sh"
    ...
    loaded = subprocess.run(  # noqa: S603
        [
            "/bin/bash",
            "-c",
            'EDEN_OPS_LIBRARY_ONLY=true source "$5"; load_dotenv_file "$1" false; '
...
def test_deployment_generation_is_complete_and_separates_credentials(
    ...
    values = {
        ...
        "DB_NAME": "eden",
        "DB_USER": "reader",
        "DB_PASSWORD": "read-only-test",
        "INGESTION_DB_USER": "writer",
        "INGESTION_DB_PASSWORD": "write-only-test",
        "MIGRATION_DB_USER": "migrator",
        "MIGRATION_DB_PASSWORD": "migration-only-test",
        "DB_NETWORK_MODE": "allowlist",
        "DB_ALLOWED_CIDRS": "192.0.2.1/32",
        "SCHEDULER_ENABLED": "false",
        "VPS_IP_ADDRESS": "192.0.2.2",
        "VPS_USERNAME": "deployer",
        "VPS_PASSWORD": "ssh-only-test",
        "VERCEL_DEPLOY_KEY": "vercel-only-test",
        "VPS_HOST_FINGERPRINT": "SHA256:deployment-only-test",
        "DEVELOPER_DB_USER": "developer",
        "DEVELOPER_DB_PASSWORD": "developer-only-test",
        "SSHPASS": "process-only-test",
        "LLM_API_KEY": "literal-${UNCHANGED}",
    }
    source = tmp_path / ".env"
    ...
    with pytest.raises(FileExistsError):
        generate(source, output)
    assert dotenv_values(source, interpolate=False) == values
