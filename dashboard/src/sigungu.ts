// 행정안전부 행정동코드(KIKcd_H, 2026-07-20 기준)에서 생성한 시군구 목록입니다.
// 원본: https://www.mois.go.kr/cmm/fms/FileDown.do?atchFileId=FILE_00147311ctH5-ah&fileSn=1
// 규칙: 출장소 제외, 구가 있는 시는 구 단위만 포함(KTO 집중률이 구 단위로 수집됨).
// 광주(29)와 전남(46)은 EDEN 운영 DB가 통합 이전 코드로 수집 중이라 통합 코드(12) 대신 기존 코드를 사용합니다.
...
export const sigungu: Sigungu[] = [
  ...
  { code: '2812500000', name: '제물포구' },
  { code: '2815500000', name: '영종구' },
  ...
  { code: '2827500000', name: '서해구' },
  { code: '2829000000', name: '검단구' },
...
export const sigunguFor = (sidoCode: string): Sigungu[] =>
  sigungu.filter((item) => item.code.startsWith(sidoCode.slice(0, 2)));

export const sigunguName = (code: string) =>
  sigungu.find((item) => item.code === code)?.name ?? code;
